...
def update_parameters_with_adam(parameters, grads, v, s, t, learning_rate = 0.01,
    ...
    for l in range(L):
        ...
        ### START CODE HERE ### (approx. 2 lines)
        parameters["W" + str(l+1)] = np.subtract(parameters["W" + str(l+1)], np.multiply(learning_rate, np.divide(v_corrected["dW" + str(l+1)], np.power(np.add(s_corrected["dW" + str(l+1)], epsilon), 0.5))))
        parameters["b" + str(l+1)] = np.subtract(parameters["b" + str(l+1)], np.multiply(learning_rate, np.divide(v_corrected["db" + str(l+1)], np.power(np.add(s_corrected["db" + str(l+1)], epsilon), 0.5))))
        ### END CODE HERE ###
