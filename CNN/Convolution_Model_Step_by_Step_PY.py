        ...
        for h in range(n_H):           # loop over vertical axis of the output volume
            ...
            for w in range(n_W):       # loop over horizontal axis of the output volume
                ...
                for c in range(n_C):   # loop over channels (= #filters) of the output volume
                    ...
                    biases = b[:,:,:,c]
#                     print(i, h, w, c)
#                     if i == 0 and h == 0 and w == 0 and c == 0:
#                         print("a_slice:", a_slice_prev, "weights:", weights, "biases:", biases)
#                         print(weights.shape)
#                     print(conv_single_step(a_slice_prev, weights, biases))
...
def conv_backward(dZ, cache):
    ...
    dW = np.zeros((f, f, n_C_prev, n_C)) # n_C represents No. of filters
    db = np.zeros((1, 1, 1, n_C)) # n_C represents No. of filters in that level
...
def distribute_value(dz, shape):
    ...
    average = np.multiply(np.ones((n_H,n_W)),dz)
    
    # Create a matrix where every entry is the "average" value (≈1 line)
    a = np.divide(average, (n_H * n_W))
    ### END CODE HERE ###
...
def pool_backward(dA, cache, mode = "max"):
    ...
    # stride and f denotes the stride and filter shape used for pooling from dA_prev to dA
    stride = hparameters['stride']
    f = hparameters['f']
    ...
    dA_prev = np.zeros((m, n_H_prev, n_W_prev, n_C_prev)) # shape same as A_prev
...
#                         print("a_prev_slice shape:", a_prev_slice.shape, "mask_shape:", mask.shape, "mask:", mask)
                        dA_prev[i, vert_start: vert_end, horiz_start: horiz_end,c] += np.multiply(dA[i,h,w,c],mask) #Since size reduction occurs when moving 
                                                                                                                    #from dA_prev to dA due to pooling
