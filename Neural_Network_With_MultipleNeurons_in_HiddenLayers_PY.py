...
def compute_cost(A2, Y, parameters):
    ...
    cost = -(1 / m) * (float(np.squeeze(np.dot(Y, np.log(A2).T))) + float(np.squeeze(np.dot((1 - Y), np.log(1 - A2).T))))
    ### END CODE HERE ###
...
def predict(parameters, X):
    ...
    ### START CODE HERE ### (≈ 2 lines of code)
    A2, cache = forward_propagation(X, parameters)
    predicted_binaries = np.squeeze(A2)
    predictions = np.where(predicted_binaries > 0.5, 1, 0)
    ### END CODE HERE ###
